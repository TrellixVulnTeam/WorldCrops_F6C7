...
class AugmentationExperiments(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if self.experiment == 'Experiment1':
            ...
            train ,test = self.experiment2()
            ts_data = TSAugmented(train, factor=2, feature_list=self.feature_list)
            ...
            train, _ = self.experiment3()
            ts_data = TSAugmented(train, factor=5, feature_list=self.feature_list)
            ...
            train, test = self.experiment3()
            #ts_data = DriftNoiseAug(train, factor=5, feature_list=self.feature_list)
